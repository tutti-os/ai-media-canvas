...
`@nextop-os/agent-acp-kit` 是外部 npm package 第一交付物。AIMC server、message/run persistence、Tool Gateway、UI compatibility 都在 package contract 稳定后再接入。
...
| `local-agent runtime` | 阶段 1-2 | 由 `@nextop-os/agent-acp-kit` 提供执行能力 |
| `@nextop-os/agent-acp-kit package` | 阶段 1-2 | provider、transport、parser、ACP、skill helpers |
...
- `open-design` 参考清单
- `multica` 参考清单
- `@nextop-os/agent-acp-kit` 最小公共 contract 草案
...
## 7. 阶段 1：`@nextop-os/agent-acp-kit`
...
P0 先做一个独立 npm package，不做物理多包拆分。目录必须先按可拆边界组织，避免把 provider、transport、process、AIMC host binding 混到一起。
...
- 把 `@nextop-os/agent-acp-kit` 暴露为 `local-agent runtime adapter`
...
这些文件属于 AIMC host binding，可以 import `@nextop-os/agent-acp-kit`，但 package 不能反向 import 它们。
...
先按 `open-design` 与 `multica` 的成熟链路做出 `@nextop-os/agent-acp-kit`，让 `Claude`、`Codex`、`Hermes`、`Kimi`、`Kiro` 在 package 层先跑通，再由 `Agent Run Orchestrator` 统一接入 AIMC 的 run/message/tool/skill/product semantics。
