...
flowchart LR
  ...
  LA --> PKG["@nextop-os/agent-acp-kit package"]
...
更准确地说，`aimc-tools-mcp` 是 AIMC 应用侧的 tool bridge：它从 env 里拿 `AIMC_TOOL_GATEWAY_URL`、`AIMC_TOOL_TOKEN`、`AIMC_RUN_ID`、`AIMC_CANVAS_ID`，对 local agent 暴露 MCP `tools/list` / `tools/call`，再把真实调用转发给 AIMC Tool Gateway。`@nextop-os/agent-acp-kit` 不应该内置 AIMC 业务工具，只需要原样接收并传递 `mcpServers` 配置。
...
### 8. 可复用 agent-acp-kit package

local-agent 执行层应该抽成独立 npm package，当前命名为：
...
也就是说，AIMC server 和后续普通应用只依赖 `@nextop-os/agent-acp-kit`。其他包是内部实现拆分或高级插件作者入口，不应该要求业务应用逐个理解。

推荐发布形态：

| 包名 | 使用者 | 作用 |
|---|---|---|
| `@nextop-os/agent-acp-kit` | AIMC / 上层应用 | facade 包，默认入口，内置官方 providers，并重导必要 core contracts |
...
|---|---|---|
| P0 | `@nextop-os/agent-acp-kit` | 唯一上层入口；内部按 `core/process/transports/providers` 分目录，不物理拆包 |
| P1 | 仍是 `@nextop-os/agent-acp-kit` | 增加 Claude / generic ACP / fake provider，继续内置官方 providers |
...
P0/P1 不建议单独发布 `@aimc/local-agent-adapter-codex`、`@aimc/local-agent-adapter-claude` 这类官方 adapter 包。Codex / Claude / generic ACP providers 先放进 `@nextop-os/agent-acp-kit` 一个包里，通过内部目录分层保持边界：

```text
@nextop-os/agent-acp-kit/src/
  core/
  process/
  transports/acp/
  providers/codex/
...
P0 建议只有一个外部 npm package `@nextop-os/agent-acp-kit`，内部目录按 `core/`、`process/`、`transports/acp/`、`providers/codex/` 拆。等 API 稳定且确实有外部生态需求后，再决定是否物理拆成多个 npm 包。
...
- `@nextop-os/agent-acp-kit` 作为最终 package 名：使用 Nextop OS scope，同时突出 Agent Client Protocol / ACP 生态定位；README 需说明它也桥接 Codex / Claude 这类 CLI-first provider。
...
ACP 不应该散落在 AIMC server runtime 里。它应作为 `@nextop-os/agent-acp-kit` 里的一个 transport：
...
11. 上层默认只依赖 `@nextop-os/agent-acp-kit` facade。P0/P1 官方 adapters 不单独拆包，先内置在 runtime 包；P0 public API 先只承诺 fake + Codex + plugin registration contract。
...
采用方案 C：Runtime Control Plane + Agent Backend Adapter 分层，并将 local-agent 执行层抽成 `@nextop-os/agent-acp-kit`。
...
- `@nextop-os/agent-acp-kit` 负责 ACP / CLI 执行、事件归一化、model detection、cancel、resume，不包含 AIMC 业务权限。
