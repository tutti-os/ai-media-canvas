...
function resolveKitRoot() {
  const candidates = [
    ...
    path.join(
      rootDir,
      "apps/server/node_modules/@tutti-os/agent-acp-kit/package.json",
    ),
    path.join(
      rootDir,
      "server/node_modules/@tutti-os/agent-acp-kit/package.json",
    ),
  ];
  ...
  for (const base of [
    ...
    try {
      ...
      return path.dirname(
        require.resolve("@tutti-os/agent-acp-kit/package.json"),
      );
    } catch {
      // try next base
    }
  ...
  throw new Error(
    "[patch-agent-acp-kit-base] could not resolve @tutti-os/agent-acp-kit",
  );
}
...
const acpPermissionsPath = path.join(
  kitRoot,
  "dist/transports/acp/acp-permissions.js",
);
