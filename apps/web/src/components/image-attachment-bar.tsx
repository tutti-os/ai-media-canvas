...
export function ImageAttachmentBar({
  ...
  return (
    <div className="flex items-center gap-2 overflow-x-auto px-2 py-1.5">
      {attachments.map((att) => (
        <div key={att.id} className="group relative h-14 w-14 shrink-0">
          <div className="relative h-full w-full overflow-hidden rounded-lg border border-border bg-muted">
            ...
            {att.error && (
              <div
                ...
                onClick={
                  onRetry && att.file ? () => onRetry(att.id) : undefined
                }
                onKeyDown={
                  onRetry && att.file
                    ? (event) => {
                ...
                role={onRetry && att.file ? "button" : undefined}
                tabIndex={onRetry && att.file ? 0 : undefined}
                title={att.error}
              >
                <svg
                  aria-hidden="true"
