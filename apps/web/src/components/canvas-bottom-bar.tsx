    ...
    >
      <div className="flex items-center gap-0.5 rounded-full bg-card/90 backdrop-blur-lg border border-border px-1 py-1 shadow-card">
        ...
        <button
          ...
          aria-label={t("bottomBar.backgroundColor")}
        >
          {bgColor === "transparent" ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-foreground">
              {t("bottomBar.backgroundColor")}
            </span>
            <button
              type="button"
              className="flex h-5 w-5 items-center justify-center rounded text-muted-foreground hover:text-foreground transition-colors"
              onClick={() => setBgPickerOpen(false)}
              aria-label={t("bottomBar.closeColorPicker")}
            >
          ...
          <div className="flex flex-wrap items-center gap-2">
            {BG_PRESETS.map((hex) => (
              <button
                ...
                aria-label={t("bottomBar.setBackground", { color: hex })}
              />
            ))}
          </div>
          {/* Hex input row */}
